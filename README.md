...
```bash
# 在线模式（推荐）— 自动查询所有 API
python scripts/dgra_core.py --input variants.csv --tissue general

# 离线模式 — 使用本地缓存 + offline_data
python scripts/dgra_core.py --input variants.csv --tissue general --offline
...
# 输出 JSON（P1-12）
python scripts/dgra_core.py --input variants.csv --tissue general --format json --output report.json
```
