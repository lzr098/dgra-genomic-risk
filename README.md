...
| 特性 | 说明 |
|------|------|
| 🧬 **组织感知分级** | 同一变异在不同组织场景下分级不同（6 个 Profile） |
...
```json
{
  ...
  "profiles": ["general", "hematopoietic", "cardiovascular", "hepatic", "renal", "neurological"]
}
```
...
| 问题 | 原因 | 解决 |
|------|------|------|
| `Invalid tissue 'xxx'` | 组织类型不对 | 使用 general / hematopoietic / cardiovascular / hepatic / renal / neurological |
...
| Feature | Description |
|---------|-------------|
| 🧬 **Tissue-Aware Classification** | Same variant, different tiers across 6 tissue profiles |
...
| Disk | ~100MB (code + cache + offline archive) |
...
| `--tissue PROFILE` | Tissue type | `general` / `hematopoietic` / `cardiovascular` / `hepatic` / `renal` / `neurological` |
...
|-------|-------|----------|
| `Invalid tissue 'xxx'` | Wrong tissue type | Use general / hematopoietic / cardiovascular / hepatic / renal / neurological |
...
│   ├── tissue_context.json         # 6 tissue profiles
