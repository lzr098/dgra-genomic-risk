...
| 指标 | 数值 |
|------|------|
| 核心代码 | ~18,000 行（30+ 个模块） |
| 测试代码 | ~6,500 行（5 层：L1 单元 + L2 集成 + L3 功能 + L4 性能 + L5 边界/异常） |
...
pip install -r requirements.txt   # aiohttp, vcfpy, openpyxl
```
...
│  Preflight 健康检查 · 两阶段管线优化                    │
└──────────────────────────────────────────────────────┘
```

**v0.10.0 架构升级 — God Module 拆分**：`dgra_core.py`（2,098 行）拆分为 6 个独立模块：

| 模块 | 职责 |
|------|------|
| `gpa_pipeline.py` | Pipeline 主流程（async 编排、Step 1~7 调度） |
| `gpa_tier_classifier.py` | 三级分级 + 证据链 + ACMG/NMD/Missense 逻辑 |
| `gpa_report.py` | Markdown/JSON 报告生成 |
| `gpa_phaser.py` | 相位分析（GATK/distance/trio） |
| `gpa_multi_hit.py` | 多基因命中检测 |
| `gpa_qc.py` | QC 检查与 flag 管理 |

`dgra_core.py` 保留为入口（~200 行），`from ... import *` 重新导出所有公共 API，向后兼容。新增 Workflow-as-Code（`gpa_workflow.py`）作为流程编排的单一事实来源。新增 Preflight 健康检查（`gpa_preflight.py`）在每次分析前确认依赖就绪。新增两阶段管线（`gpa_two_phase.py`）将 API 调用量减少 50-200x。
...
大型 VCF（>5,000 变异）的 API 调用量可从数千次降至数十次：

```
Phase 1: 快速本地 triage（仅 VEP 注释 + 本地基因列表）
    → 过滤掉 >95% 的常见 SNP / 低影响变异
Phase 2: API enrichment 仅对 Tier 1/2 候选变异
    → gnomAD、SpliceAI、表型 LLM 仅查候选
```

典型 germline VCF 的 API 调用量减少 **50-200x**，Phase 1 耗时 <30 秒。
...
### 🛡️ Preflight 健康检查（v0.10.1+）

每次分析前自动检查六大类依赖就绪状态：

| 检查项 | 内容 |
|--------|------|
| Python 依赖 | aiohttp, vcfpy, yaml 等 |
| 在线 API 连通性 | Ensembl, UniProt, GTEx, gnomAD, SpliceAI 等 8 个 API |
| 本地文件/目录 | references/, cache/, 配置文件 |
| 磁盘空间 | cache 目录所在分区 |
| 网络/代理 | 直连 vs 代理可用性 |

未就绪项自动标记，可选跳过（可选功能降级）或切换离线模式。

### 🏗️ Workflow-as-Code（v0.11.0+）

`gpa_workflow.py` 定义了所有 GPA 执行必须遵循的标准流程，是流程编排的单一事实来源：

- 每个步骤声明：模块、函数、所需 API、跳过条件、失败处理
- 支持 `abort` / `warn` / `fallback` / `skip` 四种失败策略
- 审查门（`gpa_review_gate.py`）确保流程修改经过确认

### 🧪 L1~L5 分层测试体系（v0.9.4+）

| 层级 | 类型 | 覆盖范围 | 文件 |
|------|------|---------|------|
| **L1** | 单元测试 | 独立函数、边界条件、异常处理 | `tests/test_l1_unit.py` |
| **L2** | 集成测试 | 模块间交互、数据流、API 适配 | `tests/test_l2_integration.py` |
| **L3** | 功能测试 | 端到端场景、分级正确性、报告生成 | `tests/test_l3_functional.py` |
| **L4** | 性能测试 | 大 VCF 处理速度、内存占用、API 并发 | `tests/test_l4_performance.py` |
| **L5** | 边界/异常 | 网络中断、空输入、畸形数据、极端参数 | `tests/test_l5_edge_boundary.py` |

**测试状态**：v0.10.0 全量回归 29/29 通过，v0.9.5 Patch 2 外部审查修复 6 项全部验证。
...
可选：
  ...
  --two-phase                启用两阶段管线（大型 VCF 优化，v0.10.1+）
  --preflight                分析前执行健康检查（v0.10.1+）
...
│   ├── gpa_workflow_runner.py      # 工作流执行器
│   ├── gpa_workflow_pm.py          # 工作流项目管理
│   ├── gpa_review_gate.py          # 审查门
│   ├── gpa_proxy_routes.py         # 代理路由管理
...
│   ├── dgra_build_state.py         # 构建状态持久化
│   ├── dgra_clinvar.py             # ClinVar 专用查询
│   └── version.py                  # 版本管理
├── references/
...
│   └── offline_data/                # 离线查询归档
├── tests/                           # ~6,500 行测试
│   ├── test_l1_unit.py              # L1 单元测试
│   ├── test_l2_integration.py       # L2 集成测试
│   ├── test_l3_functional.py        # L3 功能测试
│   ├── test_l4_performance.py       # L4 性能测试
│   ├── test_l5_edge_boundary.py     # L5 边界/异常测试
...
│   ├── test_blackbox_gpa_v010.py    # v0.10.0 黑盒测试
│   ├── test_e2e_gpa_v010.py         # v0.10.0 E2E 测试
│   └── test_vep_reannotation*.py    # VEP 重注释测试
├── docs/
