...
| **假阳性过滤（v0.5.1）** | ClinVar良性排除、X连锁女性修正、同义排除、C末端截短修正 |

### v0.5.1 假阳性优化（6项）

| 优化项 | 效果 | 适用场景 |
|:---|:---|:---|
| **ClinVar 良性排除** | LOW/MODERATE + ClinVar良性 → 不计入multi-hit | 减少常见多态性假阳性 |
| **X连锁女性修正** | chrX + 杂合 + haplosufficient → Tier降级 | 女性供者/患者 |
| **C末端截短修正** | HIGH + 良性 + 终止位置≥280aa → 排除 | 良性截短变异 |
| **同义变异排除** | LOW + synonymous → 永远不计入 | 无功能影响变异 |
| **基因家族冗余** | SLC25A5/ANT、CYP2D6、SLC22A1/OCT → 降级 | 有paralog补偿的基因 |
| **HLA 排除** | HLA基因不纳入multi-hit升级 | 正常免疫多态性 |

**效果**：Tier-1假阳性降低 91%（291→26，实际数据验证）
...
| 版本 | 日期 | 主题 |
|------|------|------|
| **v0.5.1** | **2026-05-21** | **假阳性大幅优化：ClinVar良性排除、X连锁女性修正、同义排除、C末端截短修正、基因家族冗余、HLA排除** |
| v0.5.0 | 2026-05-21 | P0输入层 + P1核心引擎重构 |
